...
{
    GUIUtil::setClipboard(GUIUtil::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    l2->setText(GUIUtil::formatAmountWithUnit(nAmount));                      // Amount
    l3->setText(GUIUtil::formatAmountWithUnit(nPayFee));                      // Fee
    l4->setText(GUIUtil::formatAmountWithUnit(nAfterFee));                    // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));  // Bytes
    l8->setText(GUIUtil::formatAmountWithUnit(nChange));                      // Change
    if (nPayFee > 0)
    {
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show Riecoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, GUIUtil::formatAmount(out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, GUIUtil::formatAmount(nSum));
