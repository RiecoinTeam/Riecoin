...
class WalletMiniscriptDecayingMultisigDescriptorPSBTTest(BitcoinTestFramework):
    ...
    def _get_xpub(wallet):
        ...
        pkh_descriptor = next(filter(lambda d: d["desc"].startswith("tr(") and not d["internal"], wallet.listdescriptors()["descriptors"]))
        ...
        return pkh_descriptor["desc"].split("tr(")[1].split(")")[0].replace("/0/*", "/<0;1>/*")
