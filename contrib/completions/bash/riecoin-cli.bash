# bash programmable completion for riecoin-cli(1)
# Copyright (c) 2012-present The Bitcoin Core developers
# Copyright (c) 2013-present The Riecoin developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $riecoin-cli for RPC
_riecoin_rpc() {
...
_riecoin_cli() {
    ...
    # save and use original argument to invoke riecoin-cli for -help, help and RPC
    # as riecoin-cli might not be in $PATH
    riecoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($riecoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_riecoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _riecoin_cli riecoin-cli
